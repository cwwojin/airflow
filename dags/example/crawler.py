...
REST_API_KEY = "d1e46313a0971e6d11661f477b0dd214"
headers = {"Authorization": f"KakaoAK {REST_API_KEY}"}  # 요청 헤더

KEYWORD_LOCAL_URL = (
    "https://dapi.kakao.com/v2/local/search/keyword.json?query={}&radius=1000"
)
...
def crawl_data():
    ...
    for key in keywords:  # 각 지하철 역에 대해
        response = requests.get(
            KEYWORD_LOCAL_URL.format(key + " 맛집"), headers=headers
        )
